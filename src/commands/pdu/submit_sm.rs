...
impl DecodeWithLength for SubmitSm {
    ...
    where
        Self: Sized,
    {
        let serivce_type = tri_decode!(ServiceType::decode_from(reader), SubmitSm, service_type);
        let source_addr_ton = tri_decode!(Ton::decode_from(reader), SubmitSm, source_addr_ton);
        let source_addr_npi = tri_decode!(Npi::decode_from(reader), SubmitSm, source_addr_npi);
        let source_addr = tri_decode!(COctetString::decode_from(reader), SubmitSm, source_addr);
        let dest_addr_ton = tri_decode!(Ton::decode_from(reader), SubmitSm, dest_addr_ton);
        let dest_addr_npi = tri_decode!(Npi::decode_from(reader), SubmitSm, dest_addr_npi);
        let destination_addr = tri_decode!(
            COctetString::decode_from(reader),
            SubmitSm,
            destination_addr
        );
        let esm_class = tri_decode!(EsmClass::decode_from(reader), SubmitSm, esm_class);
        let protocol_id = tri_decode!(u8::decode_from(reader), SubmitSm, protocol_id);
        let priority_flag = tri_decode!(PriorityFlag::decode_from(reader), SubmitSm, priority_flag);
        ...
        let data_coding = tri_decode!(DataCoding::decode_from(reader), SubmitSm, data_coding);
        let sm_default_msg_id = tri_decode!(u8::decode_from(reader), SubmitSm, sm_default_msg_id);
        let sm_length = tri_decode!(u8::decode_from(reader), SubmitSm, sm_length);
        ...
        let tlvs = tri_decode!(Vec::<TLV>::decode_from(reader, tlvs_length), SubmitSm, tlvs);
...
mod tests {
    ...
    fn default_encode_decode() {
        crate::ende::tests::default_encode_decode_with_length::<SubmitSm>();
    }
}
